<template>
  <div>
    <van-sticky>
      <van-nav-bar title="公共网盘" left-text="返回" left-arrow @click-left="onClickLeft" />
    </van-sticky>
...
<script>
import { getPublicNetDisk } from '../../api/application.apis'
export default {
  name: 'networkDisk',
  data() {
    return {
  ...
  methods: {
    async getList(params) {
      let res = await getPublicNetDisk(this.params)
      if (res.code === '0') {
